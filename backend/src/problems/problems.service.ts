...
export class ProblemsService {
  ...
  async generate(dto: GenerateProblemDto) {
    this.logger.log(
      `AI generation started — prompt: "${dto.prompt.substring(0, 80)}..."`,
    );
    ...
    try {
      const persisted = await this.prisma.problem.create({
        data: {
          ...
          hiddenTestCases:
            problem.hiddenTestCases as unknown as Prisma.InputJsonValue,
          visibleTestCases:
            problem.visibleTestCases as unknown as Prisma.InputJsonValue,
          hints: problem.hints,
      ...
      this.logger.log(
        `Database persistence completed — slug: "${persisted.slug}", id: "${persisted.id}"`,
      );
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        ...
        if (error.code === "P2000" || error.code === "P2006") {
          throw new BadRequestException(
            "Invalid data format for database persistence.",
          );
        }
