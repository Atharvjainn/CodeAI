...
export class AiService {
  ...
  constructor(
    ...
    this.maxRetries = Number(this.configService.get<string>('AI_MAX_RETRIES', '2'));
    this.rateLimitMax = Number(this.configService.get<string>('AI_RATE_LIMIT_MAX', '10'));
    this.rateLimitTtl = Number(this.configService.get<string>('AI_RATE_LIMIT_TTL', '60000'));
  }
  ...
  async generateProblem(dto: GenerateProblemDto): Promise<Problem> {
    ...
    const lastError: { attempt: number; message: string } = { attempt: 0, message: '' };

    for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
      try {
        if (attempt > 0) {
          this.logger.warn(`Retry attempt ${attempt}/${this.maxRetries} — ${lastError.message}`);
          await this.sleep(attempt * 1000);
        }

        this.logger.debug(`LLM request — prompt: "${dto.prompt.substring(0, 80)}...", language: ${language}`);

        const llmResponse = await this.llm.generate(fullPrompt);
        const content = llmResponse.content;

        this.logger.debug(`LLM response received — finish_reason: ${llmResponse.finishReason}`);
        ...
        if (!parsed) {
          ...
          this.logger.warn(`Parse failed — raw response snippet: "${content.substring(0, 200)}..."`);
          continue;
        }
        ...
        if (result.success) {
          this.logger.log(`Problem generated — slug: "${result.data.slug}", difficulty: ${result.data.difficulty}`);
          return result.data;
        }
    ...
    if (lastError.message.includes('fetch') || lastError.message.includes('timeout') || lastError.message.includes('abort')) {
      throw new GatewayTimeoutException('LLM provider unreachable after retries');
    }
  ...
  private checkRateLimit(): void {
    const now = Date.now();
    this.requestTimestamps = this.requestTimestamps.filter((t) => now - t < this.rateLimitTtl);
