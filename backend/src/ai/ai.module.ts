    ...
    AiService,
    {
      ...
      useFactory: (configService: ConfigService) => {
        ...
        const commonConfig = {
          ...
          model: configService.get<string>('AI_MODEL', 'llama-3.3-70b-versatile'),
          temperature: Number(configService.get<string>('AI_TEMPERATURE', '0.7')),
          maxTokens: Number(configService.get<string>('AI_MAX_TOKENS', '4000')),
          timeoutMs: Number(configService.get<string>('AI_TIMEOUT_MS', '30000')),
        };

        if (provider === 'gemini') {
          return new GeminiProvider({
            ...commonConfig,
            model: configService.get<string>('AI_MODEL', 'gemini-2.0-flash'),
          });
        }
